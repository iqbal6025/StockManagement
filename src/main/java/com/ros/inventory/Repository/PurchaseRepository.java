...
public interface PurchaseRepository extends JpaRepository<PurchaseOrder,UUID>
{
	...
	@Query(value="select * from purchase_order where purchase_order_status =:status Order By purchase_order_status DESC limit 1",nativeQuery=true)
    PurchaseOrder getLastSession(@Param("status") String status);

	@Query(value="select * from purchase_order where purchase_order_status =:status and purchase_order_date >=:date Order By purchase_order_status DESC limit 1",nativeQuery=true)
